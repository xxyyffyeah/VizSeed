...
// 构建堆叠条形图
function buildColumnStackedChart() {
...
// 执行并保存
try {
  const { vizSeedDSL, vchartSpec } = buildColumnStackedChart();
  ...
  const specData = {
    ...
    description: '堆叠垂直条形图 - 显示各店铺按城市堆叠的销售额和利润',
    spec: vchartSpec,
  ...
  console.log('COLUMN_STACKED图规范生成成功！');
} catch (error) {
  console.error('生成COLUMN_STACKED图规范失败:', error);
  process.exit(1);
}