...
// 执行并保存
try {
  const { vizSeedDSL, vchartSpec } = buildAreaPercentChart();
  ...
  const specData = {
    ...
    description: '百分比面积图 - 显示各产品线销售额占比的趋势变化',
    spec: vchartSpec,
  ...
  console.log('AREA_PERCENT图规范生成成功！');
} catch (error) {
  console.error('生成AREA_PERCENT图规范失败:', error);
  process.exit(1);
}