...
// 执行并保存
try {
  const { vizSeedDSL, vchartSpec } = buildColumnPercentChart();
  ...
  const specData = {
    ...
    description: '百分比垂直条形图 - 显示各店铺按城市的销售额和利润百分比占比',
    spec: vchartSpec,
  ...
  console.log('COLUMN_PERCENT图规范生成成功！');
} catch (error) {
  console.error('生成COLUMN_PERCENT图规范失败:', error);
  process.exit(1);
}